...
public:
	...
	template<typename T> void processFrame(T* image_in, float exposure_time, float factor=1, bool photoAllowed =1);
...
public:
	...
	template<typename T>
	ImageAndExposure* undistort(const MinimalImage<T>* image_raw, float exposure=0, double timestamp=0, float factor=1, bool photometricAllowed =1) const;
